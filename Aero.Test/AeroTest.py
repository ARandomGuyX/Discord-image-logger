...
clear()
system('title ARandomGuy')

print(pyfiglet.figlet_format("ARandomGuy", font="slant"))
print("1. Download File.\n")

auto = int(input("Option: "))

if auto == 1 or auto == 2 or auto == 3 or auto == 4:
    vidUrl = input("Click Enter) ")
...
def title1():  # Update the title IF option 1 was picked.
    ...
    while True:
        time_elapsed = strftime('%H:%M:%S', gmtime(time() - start))
        system(f'title ARandomGuy ^| Software Downloads : {beautify(Views)} ^| Elapsed Time: {time_elapsed}')
...
def title2():  # Update the title IF option 2 was picked.
    ...
    while True:
        time_elapsed = strftime('%H:%M:%S', gmtime(time() - start))
        system(f'title ARandomGuy ^| Hearts Sent: {beautify(Hearts)} ^| Elapsed Time: {time_elapsed}')
...
def title3():  # Update the title IF option 3 was picked.
    ...
    while True:
        time_elapsed = strftime('%H:%M:%S', gmtime(time() - start))
        system(f'title ARandomGuy ^| Followers Sent: {beautify(Followers)} ^| Elapsed Time: {time_elapsed}')
...
def title4():  # Update the title IF option 1 was picked.
    ...
    while True:
        time_elapsed = strftime('%H:%M:%S', gmtime(time() - start))
        system(f'title ARandomGuy ^| Shares Sent: {beautify(Shares)} ^| Elapsed Time: {time_elapsed}')
...
def loop1():
    ...
    try:
        ...
        print("[-] The Download Was Created")
        driver.refresh()
        loop1()
...
if auto == 1:
    driver.get("https://img1.wsimg.com/blobby/go/79bfe177-2a93-41ae-a7db-b1c67bba839f/downloads/Software.zip?ver=1651377314862")
